...
w = owlready2.World(filename=os.path.join(CachePath,'firstDirect.sqlite3'), exclusive=False)
print("FirstDirect")
onto1 = w.get_ontology('https://raw.githubusercontent.com/inab/OEB-ontologies/master/oebDataFormats.owl').load()
w.save()
onto1.save()
...
w = owlready2.World(filename=os.path.join(CachePath,'firstIndirect1.sqlite3'), exclusive=False)
print("FirstIndirect1")
onto1 = w.get_ontology('https://w3id.org/oebDataFormats/').load()
w.save()
onto1.save()
res1 = onto1.search(label = "participant")
import pprint
...
w = owlready2.World(filename=os.path.join(CachePath,'firstIndirect2.sqlite3'), exclusive=False)
